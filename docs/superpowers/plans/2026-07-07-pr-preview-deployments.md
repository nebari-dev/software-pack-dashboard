...
|---|------|-------|--------------|----------|
| 1 | A same-repo PR touching `site/**` gets a preview build + deploy | On such a PR, the `preview-deploy` job runs and succeeds; wrangler reports a `pages deploy` to branch `pr-<n>` | narrated: the implementation PR's own Actions run - paste the successful job log showing the `pages deploy … --branch=pr-<n>` step | ✅ PR #18, run 28875732089, `preview` job success (54s). Deploy step log: `npx wrangler pages deploy site/dist --project-name=software-pack-dashboard --branch=pr-18` → `✨ Success! Uploaded 7 files`, `✨ Deployment complete!` |
| 2 | A working preview URL is posted to the PR | A PR comment contains `https://pr-<n>.software-pack-dashboard.pages.dev`; `curl -sI` that URL returns HTTP 200 | narrated: text of the comment + `curl -sI` transcript showing `200` | ✅ Sticky comment on PR #18: `**Preview deploy:** https://pr-18.software-pack-dashboard.pages.dev`. `curl -sSI` → `HTTP/2 200`. |
| 3 | The preview renders the PR's changes, not production | A deliberate visible change in the PR (marker string in `dashboard.css` or content) is present in the deployed preview | narrated: `curl -s <preview-url>/_astro/*.css \| grep <marker>` (or rendered HTML) showing the change | ✅ Marker `pr-preview-check-20260707` found in deployed `/_astro/common.dowBPYMM.css` (bundle hash differs from prod's `common.dowBPYMM` vs local `B6N4Psfj`, confirming a branch-specific build). Absent from CSS after marker removal. |
| 4 | Re-pushing updates one sticky comment, not many | After a second push to the PR, exactly one comment carries the `<!-- preview-url -->` marker, updated to the newest deploy | narrated: `gh pr view --json comments` count of marker comments == 1 after 2 pushes | ✅ After force-push re-run (run 28876083684), marker-comment count == 1; comment `Built from` updated in place `7840ee4` → `a52ebea`. |
| 5 | A PR that does not touch `site/**`/`generate.py` triggers no preview | On a Python-only or CI-only PR, no `preview-deploy` run appears | narrated: `gh pr checks` / Actions list for such a PR showing no preview-deploy run | ⚠️ Path filter verified static: `on.pull_request.paths = [site/**, generate.py, .github/workflows/preview-deploy.yml]`. Sibling CI-only PR #15 shows no `preview` check. Full runtime confirmation on a non-site PR is only possible once the workflow is on `main` (post-merge). **user-waived 2026-07-07: static path-filter verified; runtime self-confirms on the first non-site PR after merge.** |
| 6 | A fork PR is skipped cleanly - no failed check, no secret use | The job `if` gates on same-repo; a fork PR shows the job skipped (neutral), never a red X, never touches secrets | narrated: inspect the guard expression + a fork-PR run showing "skipped"; likely a user waiver since a fork PR cannot be opened from the dev environment | ⚠️ Guard confirmed at `preview-deploy.yml:24` (job-level `if: github.event.pull_request.head.repo.full_name == github.repository`), independently verified correct by the task reviewer. A fork PR cannot be opened from the dev environment. **user-waived 2026-07-07: job-level guard confirmed correct; end-to-end fork "skipped" check accepted on the strength of the guard + standard GitHub fork-secret behavior.** |
| 7 | The preview never deploys to production or the Worker | The run only executes `pages deploy … --branch=pr-<n>` (never `--branch=main`) and never runs the Worker `deploy`; `packs.nebari.dev` is unchanged | narrated: full job log confirming only the preview branch deploy step ran | ✅ PR #18 `preview` job step list contains no Worker `deploy` step (only `Deploy preview to Cloudflare Pages`); command is `--branch=pr-18`, never `--branch=main`. `packs.nebari.dev` untouched. |

Verification gate run 2026-07-07 on PR #18. 5 of 7 journeys fully verified (1, 2, 3, 4, 7). Journeys 5 (runtime portion) and 6 (fork end-to-end) are static-verified only and await a user waiver, since neither can be exercised until the workflow is on `main` / from a fork.
